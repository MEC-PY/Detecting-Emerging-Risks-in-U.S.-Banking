# Textual Factors model core classes
# NeighborFinder  – FAISS-based nearest neighbor search (brute force + LSH)
# EmbeddingCluster – sequential / heuristic / hierarchical clustering
# TextualFactors  – LSA and LDA topic extraction within clusters
# based on zhu & wu (2024)
...
class TextualFactors():
    ...
    def perform_svd(self,doc_word_df, n_components=2):
        ...
        matrix = doc_word_df.values.astype(float)

        # L2-normalise each document (row) before SVD
        # Per Cong et al. (2024) Figure 2: SVD is applied to the
        # "normalized doc-term matrix" N, where each document's word
        # vector is L2-normalised across the cluster's vocabulary.
        row_norms = np.linalg.norm(matrix, axis=1, keepdims=True)
        row_norms[row_norms == 0] = 1.0          # protect zero-rows
